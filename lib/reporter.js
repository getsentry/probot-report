...
module.exports = class Reporter {
  ...
  async getTimeZoneForUser(user) {
    ...
    if (!commit) {
      console.debug(`Did not find commits for user "${user.login}, assuming default timezone`);
  ...
  async sendReport(user) {
    console.info(`Generating report for user "${user.login}"...`);
    const pullRequests = await this.getPullRequestsForUser(user);

    if (pullRequests.length > 0) {
      this.emitter.emit(EVENT_REPORT, user, pullRequests);
    } else {
      console.debug(`Skipping report for "${user.login}", no pull requests found`);
    }
  }
  ...
  async addUser(user, writeConfig = true) {
    ...
    if (!userConfig) {
      console.info(`Found new user "${login}", fetching details and timezone`);
      ...
      if (!details.email) {
        console.warn(`No email found for user "${login}"`);
      }
  ...
  async setupUsers() {
    ...
    if (targetType === TYPE_ORGANIZATION) {
      console.debug(`Loading all organization members for "${account.login}"`);
      ...
      console.info(`Initializing ${users.length} users for organization "${account.login}"`);
      await Promise.all(users.map(user => this.addUser(user, false)));
    } else if (targetType === TYPE_USER) {
      console.info(`Initializing account "${account.login}" as user`);
