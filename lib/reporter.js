...
module.exports = class Reporter {
  ...
  searchIssues(github, query) {
    return github.paginate(
      ...
      response => response.data.items
        .filter(issue => !this.isIgnored(issue)),
    );
  }
  ...
  async getReportForUser(user) {
    ...
    const toReview = await this.searchIssuesRateLimited(github,
      `is:open is:pr review:none review-requested:${login}`);
  ...
  async sendReport(user) {
    ...
    if (report.hasData()) {
      ...
      if (weekday <= 5) {
        ...
        this.logger.debug('We don\'t send reports on weekends');
      }
    } else {
  ...
  scheduleReport(user, time) {
    ...
    this.logger.debug(`Scheduled job for "${user.login}" job: ${rule.nextInvocationDate()}`);
  ...
  scheduleReportsForAllUsers() {
    const config = this.config.get();
    _.forEach(
      config.users,
      user => config.reportTimes.forEach(time => this.scheduleReport(user, time)),
    );
  }
  ...
  async setupUsers() {
    ...
    if (targetType === TYPE_ORGANIZATION) {
      ...
      await Promise.all(users.map(user => this.addUser(github, user, false))).then(
        () => this.scheduleReportsForAllUsers(),
      );
    } else if (targetType === TYPE_USER) {
      this.logger.info(`Initializing account "${account.login}" as user`);
      await this.addUser(github, account, false).then(() => this.reloadUsers());
    } else {
