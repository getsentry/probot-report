...
module.exports = class Reporter {
  ...
  static async githubSearch(github, query) {
  ...
  async getPullRequestsForUser(user) {
    ...
    const toReview = await Reporter.githubSearch(
      github,
      `review:none is:pr is:open review-requested:${login} org:${org}`);

    // This query returns all PRs that already have been reviewed
    // and need to be completed by the assignee
    const toComplete = await Reporter.githubSearch(
      github,
      `-review:none is:pr is:open assignee:${login} org:${org}`);

    // We can pull this out of here when we have issue or stuff we query
    // for now leave it here
    const report = new Report();
    return report.addPullRequests({ toReview, toComplete });
  }

  async sendReport(user) {
    this.logger.info(`Generating report for user "${user.login}"...`);
    const report = await this.getPullRequestsForUser(user);
