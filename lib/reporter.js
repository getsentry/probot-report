  ...
  async getPullRequestsForUser(user) {
    ...
    const toReview = await github.paginate(
      github.search.issues({
        q: `review:none is:pr is:open review-requested:${login} org:${org}`,
        per_page: 100,
      }),
    ...
    // we need to make two quries with
    // review:changes_requested and review:approved
    // since github is not supporting OR
    const changesRequested = await github.paginate(
      github.search.issues({
        q: `review:changes_requested is:pr is:open assignee:${login} org:${org}`,
        per_page: 100,
      }),
      response => response.data.items);
    const approved = await github.paginate(
      github.search.issues({
        q: `review:approved is:pr is:open assignee:${login} org:${org}`,
        per_page: 100,
      }),
      response => response.data.items);

    const toComplete = _.concat(changesRequested, approved);
