...
module.exports = class Slack {
  ...
  handleUnconnectedMessage(message) {
    ...
    this.send(message, `Ayo *${githubLoginMatch[1].toLowerCase()}*, I know who you are now. :heart:`);
    this.sendHelp(message);
  }
  ...
  sendReport(report) {
    ...
    if (newIssues.length > 0) {
      this.send(source, {
        ...
        attachments: newIssues.map(issue => Slack.createIssueAttachment(issue, { color: '#37d7e4' })),
      });
    }
  }
};
