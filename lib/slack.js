 ...
 * Event emitted when generating reports
 */
const EVENT_REQUEST_MAIL = 'slack.request.mail';
...
module.exports = class Slack {
  ...
  onRequestMail(callback) {
    this.emitter.on(EVENT_REQUEST_MAIL, callback);
    return this;
  }
  ...
  getConnectedUser(message) {
    ...
    return _.find(
      users,
      user => user.slack && user.slack.user === message.user
    );
  }
  ...
  sendHelp(src) {
    this.send(
      ...
      'Here are a few commands you can run:\n' +
        ...
        '  :four:  `email off` _you will no longer receive emails_\n' +
        '  :five:  `set <oldest|newest> first` _configure the order of PRs_\n' +
        '  :six:  `get config` _list the config for your user_\n' +
        ''
    );
  }
  ...
  handleConnectedMessage(message, user) {
    ...
    if (text === 'slack on') {
      this.send(
        message,
        ':white_check_mark: You will now receive notifications via slack.'
      );
      this.config.mergeUser(user.login, {
        slack: { ...user.slack, active: true },
      });
    } else if (text === 'slack off') {
      this.send(
        message,
        ':no_entry_sign: You no longer receive notifications via slack.'
      );
      this.config.mergeUser(user.login, {
        slack: { ...user.slack, active: false },
      });
    } else if (text === 'email off') {
      this.send(
        message,
        ':no_entry_sign: You no longer receive notifications via email.'
      );
      this.config.mergeUser(user.login, {
        email: null,
      });
      ...
      this.send(message, ':eyes: Gimme a second...');
      this.emitter.emit(EVENT_REQUEST_MAIL, user);
    } else if (emailMatch !== null) {
      this.send(
        message,
        `:white_check_mark: Updated email to *${emailMatch[0]}*`
      );
  ...
  handleUnconnectedMessage(message) {
    ...
    this.config.mergeUser(githubLoginMatch[1].toLowerCase(), { slack });
    this.send(
      message,
      `Ayo *${githubLoginMatch[1].toLowerCase()}*, I know who you are now. :heart:`
    );
    this.sendHelp(message);
  }
  ...
  sendReport(report) {
    ...
    if (toReview.length > 0) {
      this.send(source, {
        ...
        attachments: toReview.map(issue =>
          Slack.createIssueAttachment(issue, { color: '#36a64f' })
        ),
      });
    }
    ...
    if (toComplete.length > 0) {
      this.send(source, {
        ...
        attachments: toComplete.map(issue =>
          Slack.createIssueAttachment(issue, { color: '#F35A00' })
        ),
      });
    }

    const newIssues = report.getNewIssues();
    if (newIssues.length > 0) {
      this.send(source, {
        text: 'Here are issues you could *label and assign*:',
        attachments: newIssues.map(issue =>
          Slack.createIssueAttachment(issue, { color: '#37d7e4' })
        ),
      });
    }
  }
