...
module.exports = class Slack {
  ...
  send(src, message, callback) {
    console.debug('Sending slack message', message);
    if (shouldPerform()) {
  ...
  static createIssueAttachment(issue) {
  ...
  sendReport(user, pullRequests) {
    ...
    const text = 'Here are your pull requests to review:\n';
    const attachments = pullRequests.map(Slack.createIssueAttachment);
    this.send(source, { text, attachments });
  }
};
