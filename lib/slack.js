...
module.exports = class Slack {
  ...
  static createIssueAttachment(issue, color) {
  ...
  sendReport(user, pullRequests) {
    if (!user.slack.active) {
      return;
    }

    const source = user.slack;
    if (pullRequests.toReview.length === 0 && pullRequests.toComplete.length === 0) {
      this.send(source, ':tada: You have no pull requests to review! :beers:');
      return;
    }

    if (pullRequests.toReview.length > 0) {
      this.send(source, {
        text: 'Here are your pull requests to review:\n',
        attachments: pullRequests.toReview.map(issue => Slack.createIssueAttachment(issue, '#36a64f')),
      });
    }

    if (pullRequests.toComplete.length > 0) {
      this.send(source, {
        text: 'Also here are pull requests you need to finish:\n',
        attachments: pullRequests.toComplete.map(issue => Slack.createIssueAttachment(issue, '#F35A00')),
      });
    }
  }
};
