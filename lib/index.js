...
async function addReporter(github, installation) {
  ...
  if (reporters[id] == null) {
    const config = await configManager.loadConfig(github, installation.account);
    slack = new Slack()
      .onReceivedGithubProfile((githubProfile, slackUserInfo) => {
        configManager.updateUser(github, installation.account, githubProfile, slackUserInfo);
      });
    const mailer = new Mailer(config.transports.email);
    reporters[id] = new Reporter(github, installation, config)
      .onConfigChange(newConfig => configManager.writeConfig(github, installation.account, newConfig))
