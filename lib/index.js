...
async function requestReport(instance, user) {
  ...
  const pullRequests = await instance.reporter.getReportForUser(user);
  instance.slack.sendReport(user, pullRequests);
}
...
async function addReporter(robot, installation) {
  ...
  if (instances[id] == null) {
    ...
    const reporter = new Reporter(robot, installation, config)
      .onReport((user, report) => sendReport(instances[id], user, report));
    const slack = new Slack(config, logger)
      .onRequestReport(async user => requestReport(instances[id], user));
