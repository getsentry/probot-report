...
async function addReporter(github, installation) {
  ...
  if (reporters[id] == null) {
    const config = await new Config(github, installation.account, logger).load();
    const mailer = new Mailer(config.get().email, logger);
    const reporter = new Reporter(github, installation, config, logger)
