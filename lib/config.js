...
module.exports = class Config {
  ...
  async load() {
    ...
    console.info(`Loading config from ${context.owner}/${context.repo}:${context.path}`);
  ...
  async write() {
    ...
    const context = this.getContext();
    console.info(`Persisting config to ${context.owner}/${context.repo}:${context.path}`);
  ...
  save() {
    ...
    if (isDryRun()) {
      console.debug('Config write skipped due to dry run');
      return this;
    }
  ...
  mergeIn(path, data) {
    ...
    console.debug(`Merging keys {${_.keys(data).join(',')}} into config.${path.join('.')}`);
  ...
  merge(data) {
    ...
    console.debug(`Merging keys {${_.keys(data).join(',')}} into config`);
