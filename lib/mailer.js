...
function getMailer(logger = console) {
  ...
  try {
    ...
    mailer = NodeMailer.createTransport(
      sendgridTransport({ auth: { api_key: token } })
    );
    return mailer;
  } catch (e) {
...
module.exports = class Mailer {
   ...
   */
  compileTemplates() {
    const { email } = this.config.get();
    const templates = _.pickBy(email, (i, key) => /Template$/.test(key));
  ...
  async sendReport(report) {
    ...
    const { subject } = this.config.get().email;

    const html = this.templates.bodyTemplate({
      user,
      toReview: this.formatIssues(
        this.templates.toReviewTemplate,
        report.getPullRequestsToReview()
      ),
      toComplete: this.formatIssues(
        this.templates.toCompleteTemplate,
        report.getPullRequestsToComplete()
      ),
      newIssues: this.formatIssues(
        this.templates.newIssuesTemplate,
        report.getNewIssues()
      ),
    });
