...
function createMailer(token) {
  if (!token) {
    console.error('Could not configure mailer: Token missing');
    return null;
  }

  try {
    // TODO: Allow for other transports than sendgrid
    console.info('Initializing Sendgrid mailer');
...
function sendMail(data) {
  ...
  console.debug('Skipping email, no mailer configured');
...
module.exports = class Mailer {
  ...
  async sendReport(user, pullRequests) {
    if (user.email == null) {
      console.error(`Cannot send email to ${user.login}: No email found`);
      return;
    }
