...
module.exports = class Mailer {
   ...
   */
  async sendReport(user, pullRequests) {
    if (user.email == null) {
    ...
    const subject = this.templates.subjectTemplate({
      count: (pullRequests.toReview.length + pullRequests.toComplete.length),
    });

    const toReview = pullRequests.toReview.length > 0 ? this.templates.toReviewTemplate({
      issues: this.templates.issueTemplate({
        pullRequests: pullRequests.toReview,
      }),
    }) : '';

    const toComplete = pullRequests.toComplete.length > 0 ? this.templates.toCompleteTemplate({
      issues: this.templates.issueTemplate({
        pullRequests: pullRequests.toComplete,
      }),
    }) : '';
